...
namespace MyEcommerceAuthorize.Application.ContaContext.Handler;
                              ...
                              IRequestHandler<DeleteUsuarioCommand, bool>
{
    ...
    public async Task<bool> Handle(DeleteUsuarioCommand request, CancellationToken cancellationToken) => await _usuarioService.Delete(request.Id);

    public async Task<bool> Handle(AutenticateUsuarioQuery request, CancellationToken cancellationToken) => await _usuarioService.Autentique(request.Email, request.Senha);
}
